...
MODEL_B_PATH = "model_b_eyelid (2).pth"
...
# Modelo B:
# 0 = fondo
# 1 = menisco / párpado inferior según entrenamiento binario
THRESHOLD_B = 0.7
...
# ============================================================
# 5. PREDICCIÓN MODELO B - MENISCO
# ============================================================

def predict_model_B(img_rgb, threshold=0.7):
    h, w = img_rgb.shape[:2]

    tensor = prepare_tensor(img_rgb)

    with torch.no_grad():
        output = model_B(tensor)
        ...
        if isinstance(output, dict):
            if "out" in output:
                ...
                raise ValueError(f"Salida no reconocida en model_B: {output.keys()}")

        # Si el modelo no tiene sigmoid aplicado, lo aplicamos.
...
def get_iris_diameter_from_model_A(img_rgb, pred_A):
    ...
    if len(iris_cnt) >= 5:
        ...
        # Usamos eje menor para evitar inflar el diámetro.
...
# ============================================================
# 7. CALCULAR TMH DESDE MODELO B + IRIS AUTOMÁTICO
# ============================================================

def calculate_tmh_from_model_b_robust(img_rgb, pred_B, iris_diam_px):
    h, w = img_rgb.shape[:2]
    ...
    if n_cc <= 1:
        return None, img_rgb.copy(), {
            "error": "Modelo B no detectó menisco"
        }
    ...
    for i in range(1, n_cc):
        ...
        # Menisco debe estar en zona media-inferior de la imagen ocular.
        if cy < h * 0.35:
            continue
    ...
    if len(candidates) == 0:
        return None, img_rgb.copy(), {
            "error": "No hay componente válido de menisco"
        }
    ...
    for x in range(x_left, x_right + 1):
        ...
        # Evita columnas demasiado gruesas por ruido.
        if 1 <= height <= 30:
...
plt.colorbar()
plt.axis("off")
plt.title("Mapa de probabilidad Modelo B")
plt.show()

plt.figure(figsize=(10, 6))
plt.imshow(pred_B, cmap="gray")
plt.axis("off")
plt.title(f"Máscara Modelo B - threshold={THRESHOLD_B}")
plt.show()
...
SAVE_PATH = "/kaggle/working/tmh_resultado_final.png"
